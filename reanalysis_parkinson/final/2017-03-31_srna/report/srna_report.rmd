...
### Isomirs
```{r mirna-isomirs}
isoPlot(obj, column = "group")
isoPlot(obj, column = "group", type = "iso3")
isoPlot(obj, column = "group", type = "add")
isoPlot(obj, column = "group", type = "subs")
isoPlotPosition(obj, position = 6, column = "group")
```

```{r}

obj2 <- IsomirDataSeqFromFiles(files = files[rownames(design)][1:6], design = design[1:6,] , header = T, skip=0, rate = 0.1)

isoPlot(obj2, type = "subs", column = "group")

obj_iso = isoCounts(obj2, ref = T, iso5 = T, iso3 = T, add = T, subs = T, seed = T)
parsed = as.data.frame(counts(obj_iso)) %>% tibble::rownames_to_column("id") %>%
    tidyr::separate(col = id, sep = "[.]", into = c("mir","type","t5","seed",
                                                  "t3","add","mm"))
parsed = parsed[rowSums(parsed[,7:12]>1)>1,]
mm = parsed %>% filter(mm!="mm:0", !grepl("N",mm))
ref = parsed %>% filter(mm=="mm:0",add=="ad:0")

df = mm %>% dplyr::select(mir, mm, pc2:ct1) %>% 
    melt() %>% filter(value>0) %>%
    group_by(mir, mm, variable) %>% summarise(mut_exp=sum(value), mut_div=n()) %>%
    mutate(id=paste(mir, variable)) %>% 
    dplyr::select(id, mut_exp, mut_div) %>%
    left_join(ref %>% dplyr::select(mir, pc2:ct1) %>% melt() %>%
                  mutate(id=paste(mir, variable)) %>%
                  group_by(id) %>% summarise(ref_exp=sum(value)) %>%
                  dplyr::select(id, ref_exp), by="id")

 df %>%
    ggplot(aes(x=log2(ref_exp+1), y=log2(mut_exp+1),size=mut_div)) + geom_point()
...
pheatmap(assay(vst), annotation_col = design[,1:2], show_rownames = F, 
